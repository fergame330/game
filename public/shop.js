...
function buildRoundChoicePool() {
  ...
  if (!power.minigun.unlocked) {
    functions.push({
      id: "unlock-minigun",
      type: "Função",
      title: "Desbloquear Metralhadora",
      desc: "Ative com a tecla 1.",
      apply: () => {
        power.minigun.unlocked = true;
        showMessage("Metralhadora desbloqueada!", { color: "#55ccff" });
        updatePowersUI();
      },
    });
  }

  if (!power.bomb.unlocked) {
    functions.push({
      id: "unlock-bomb",
      type: "Função",
      title: "Desbloquear Explosão Tripla",
      desc: "Ative com a tecla 2.",
      apply: () => {
        power.bomb.unlocked = true;
        showMessage("Explosão Tripla desbloqueada!", { color: "#ffd700" });
        updatePowersUI();
      },
    });
  }

  if (!power.immune.unlocked) {
    functions.push({
      id: "unlock-immune",
      type: "Função",
      title: "Desbloquear Imunidade",
      desc: "Ative com a tecla 3.",
      apply: () => {
        power.immune.unlocked = true;
        showMessage("Imunidade desbloqueada!", { color: "#aaffff" });
        updatePowersUI();
      },
    });
  }

  if (powersOnCooldown() && (power.minigun.unlocked || power.bomb.unlocked || power.immune.unlocked)) {
    functions.push({
      id: "power-reset",
      type: "Função",
